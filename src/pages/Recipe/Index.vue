<template>
  <FullPageLayout>
    <div
      class="md:flex gap-6 space-y-4 lg:space-y-0"
      v-for="(item, index) in currentRecipeHits"
      :key="index"
    >
        ...
        <div class="lg:w-2/3">
          ...
          <div class="flex justify-between items-center bg-emerald-100/70 text-emerald-900 font-semibold px-3 py-2 rounded-xl mb-3 mt-4">
            ...
            <span v-if="hasSearch" class="text-xs font-normal">
              Showing {{ filteredIngredients(item.recipe.ingredients).length }} of {{ item.recipe.ingredients.length }}
            </span>
          </div>

          <div
            v-if="filteredIngredients(item.recipe.ingredients).length > 0"
            class="grid grid-cols-2 sm:grid-cols-3 gap-3"
          >
            <Ingredient :ingredients="filteredIngredients(item.recipe.ingredients)" />
          </div>
          <div
            v-else
      ...
      <div class="flex-1 h-fit border border-gray-100 bg-white shadow-sm rounded-3xl p-5 pt-4 mt-5 md:mt-0">
        <div class="flex justify-between items-center pb-2 border-b border-gray-100">
          ...
          <span v-if="hasSearch" class="text-xs text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full font-medium">
            {{ filteredNutrients(item.recipe.totalNutrients).length }}
          </span>
        </div>
        <ul class="pl-2 divide-y divide-gray-100 mt-2 max-h-96 overflow-y-auto">
          <li
            class="flex gap-1 justify-between py-2 text-xs"
            v-for="nutrient in filteredNutrients(item.recipe.totalNutrients)"
            :key="nutrient.label"
